...
RSpec.describe Order, type: :model do
  describe 'create' do
    ...
    let!(:item1)       { create(:item, category_id: category1.id) }
    let!(:item2)       { create(:item, category_id: category2.id) }
    let!(:item3)       { create(:item, category_id: category3.id) }
    let!(:item4)       { create(:item, category_id: category2.id) }
